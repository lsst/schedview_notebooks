...
for SCHEDVIEW_VISIT_ORIGIN in ${SCHEDVIEW_INSTRUMENTS} ; do
  ...
  cp "${COMPARE_NIGHT_FNAME_BASE}.html" "${COMPPRE_PUB_FNAME}"
  chmod 644 "${COMPPRE_PUB_FNAME}"
  for SCHEDULER_GROUP_USER in ${SCHEDULER_GROUP_USERS}; do setfacl -m ${SCHEDULER_GROUP_USER}:rw ${COMPPRE_PUB_FNAME} ; done
